...
class BookingViewTest(SetUpMixin, UserMixin, BookingMixin, TestCase):
    ...
    def test_create(self):
        data = {'name': 'pete', 'number_of_guest': 4,
                'bookings_date': '2023-03-04'}
...
class SingleBookingViewTest(SetUpMixin, UserMixin, SingleBookingMixin, TestCase):
    ...
    def test_partial_update(self):
        data = json.dumps(
            {'number_of_guest': 6, 'bookings_date': '2023-03-06'})
    ...
    def test_update(self):
        data = json.dumps(
            {'name': 'will', 'number_of_guest': 6, 'bookings_date': '2023-03-06'})
    ...
    def test_delete(self):
        ...
        self.assertEqual(Booking.objects.filter(
            pk=self.booking.pk).exists(), False)
...
class SingleMenuItemViewTest(SetUpMixin, UserMixin, SingleMenuItemMixin, TestCase):
    ...
    def test_update(self):
        data = json.dumps(
            {'title': 'Apple Juice', 'price': 3.85, 'inventory': 7})
        response = self.client.put(
            reverse('api:menu-detail', kwargs={'pk': self.menu_item.pk}),
            data=data,
            content_type='application/json',
    ...
    def test_delete(self):
        ...
        self.assertEqual(Menu.objects.filter(
            pk=self.menu_item.pk).exists(), False)
